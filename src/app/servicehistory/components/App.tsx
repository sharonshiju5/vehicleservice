...
function App() {
  ...
  const filteredBookings = bookings.filter(booking => 
    booking.bookingStatus === activeTab
  )
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-4">
        ...
        {filteredBookings.length === 0 ? (
          ...
          <div key={booking._id} className="bg-white rounded-lg p-4 mb-4 shadow-sm">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center">
                <div className="w-12 h-12 bg-gray-200 rounded mr-3 flex items-center justify-center">
                  <span className="text-xs font-medium">{booking.subCategoryId?.name?.charAt(0) || 'S'}</span>
                </div>
                <div>
                  <h3 className="font-semibold text-gray-900">{booking.subCategoryId?.name || 'Service'}</h3>
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${
                booking.bookingStatus === 'pending' ? 'bg-orange-100 text-orange-600' :
            ...
            <button className={`w-full py-2 rounded-lg font-medium ${
              ...
              {booking.bookingStatus === 'pending' ? 'Track Work' :
               booking.bookingStatus === 'confirmed' ? 'Review' :
               'Re-Book Service'}
            </button>
          </div>
        ))
